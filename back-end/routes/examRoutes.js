...
router.get("/all", protect(["trainer", "admin", "learner"]), getAllExams);

// ✅ Fetch questions for a specific exam
router.get("/:examId/questions", protect(["trainer", "admin", "learner"]), getExamQuestions);

// ✅ Enroll in an Exam (Learner)
router.post("/enroll/:examId", protect(["learner","admin"]), enrollExam);

// ✅ Get Enrolled Exams for a Learner
router.get("/enrolledExam", protect(["learner","admin"]), getEnrolledExams);

// ✅ Submit Exam Result
router.post("/submit-result", protect(["learner","trainer","admin"]), submitResult);

// ✅ Get Submitted Results for a User
router.get("/submitted-results", protect(["learner", "trainer","admin"]), getSubmittedResults);
...
router.get("/:examId/certificate", protect(["learner","trainer","admin"]), generateCertificate);
