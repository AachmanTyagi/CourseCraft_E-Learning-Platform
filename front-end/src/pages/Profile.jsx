...
const Profile = () => {
  ...
  const { enrolledCourses, getEnrolledCourses } = useCourse();
  const { results, loading: resultsLoading, fetchResults } = useExam();
  ...
  useEffect(() => {
    ...
    getEnrolledCourses();
    fetchResults();
  }, [fetchCurrentUser, getEnrolledCourses, fetchResults]);
  ...
  const handleProfileUpdate = async () => {
    if (selectedFile && currentUser) {
      ...
      try {
        await updateUser({ id: currentUser._id, updates: formData, profilePicture: selectedFile });
        setPreview(null);
